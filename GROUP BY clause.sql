# Assignment-4 by Tushti Gupta SODS:-
use sale;
...
Select Religion,avg(Quantity) as "Average sales" from sales
group by Religion;
...
Select CustomerId, count(*) As "Total Sales" from sales
group by CustomerID;
...
Select CustomerID,religion, sum(Quantity*UnitPrice) AS "Total Sales" from sales
group by CustomerID,religion;

# Q11- Query to calculate the total sales for each month.
Select month(SaleDate), sum(Quantity*UnitPrice) AS "Total Sales" from sales
group by month(SaleDate);

# Q12- Find the regions where the average unit price is more than 200.
Select Religion,avg(UnitPrice) AS "Avg Unit Price"from sales
group by Religion
...
Select CustomerID AS Purchase from sales group by customerid having count(*) > 2;
